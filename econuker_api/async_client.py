...
class AsyncClient:
    ...
    def __init__(self, auth_token: str = None, beta: bool = False):
        ...
        self.items = AsyncItemsAPI(self.auth_token, self.base_url, self.auth_level)
        self.servers = AsyncServersAPI(self.auth_token, self.base_url, self.auth_level)
        self.auth = AsyncAuthAPI(self.auth_token, self.base_url, self.auth_level)

        if self.auth_token is not None:
            token_future = asyncio.run_coroutine_threadsafe(self.auth.verify(self.auth_token), asyncio.get_event_loop())
...
class AsyncAuthAPI:
    """
    API wrapper for interacting with the auth endpoint.
    ...
    def __init__(self, auth_token, base_url, auth_level):
...
class AsyncServersAPI:
    """
    API wrapper for interacting with the servers endpoint.
    ...
    def __init__(self, auth_token: str, base_url: str, auth_level: str):
        self.auth_token: str = auth_token
        self.base_url: str = base_url
        self.auth_level: str = auth_level
...
class AsyncItemsAPI:
    """
    API wrapper for interacting with the items endpoint.
